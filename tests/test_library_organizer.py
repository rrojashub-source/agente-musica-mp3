...
class TestLibraryOrganizer:
    ...
    def test_01_organizer_class_exists(self):
        ...
        if self.organizer_class is None:
            pytest.fail("LibraryOrganizer not found - implement src/core/library_organizer.py")

        assert self.organizer_class is not None

    def test_02_organizer_builds_path_from_template(self):
        """Test path generation from template"""
        if self.organizer_class is None:
            pytest.skip("Organizer not implemented")
        ...
        expected = os.path.join(self.temp_dir, "Queen", "A Night at the Opera (1975)", "01 - Bohemian Rhapsody.mp3")
    ...
    def test_03_organizer_sanitizes_folder_names(self):
        ...
        song = {"title": "Song?", "artist": "Artist/Band", "album": "Album:Name", "year": 2020, "track": 1}
        ...
        assert "Artist_Band" in relative_path  # '/' was replaced
        assert "Album_Name" in relative_path  # ':' was replaced
    ...
    def test_08_organizer_handles_duplicate_names(self):
        ...
        assert "song" in os.path.basename(new_path)
    ...
    def test_11_organizer_processes_1000_files_fast(self):
        ...
        assert elapsed < 30.0, f"Organization too slow: {elapsed:.2f}s"
