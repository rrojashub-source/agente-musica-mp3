...
class TestMusicBrainzClient:
    ...
    def setup(self, tmp_path):
        ...
        self.test_output = tmp_path / "cover.jpg"
    ...
    def test_client_class_exists(self):
        ...
        if self.client_class is None:
            pytest.fail("MusicBrainzClient class not found - implement src/api/musicbrainz_client.py")
    ...
    def test_client_initialization_sets_user_agent(self):
        ...
        if self.client_class is None:
            pytest.fail("MusicBrainzClient class not found")

        # Create client
        client = self.client_class(app_name="NexusMusicManager", app_version="1.0")

        # Verify user agent is set (should have app name)
        assert hasattr(client, "app_name")
        assert client.app_name == "NexusMusicManager"
    ...
    def test_search_recording_returns_results(self):
        ...
        with patch("musicbrainzngs.search_recordings") as mock_search:
                ...
                "recording-list": [
                    {
                        ...
                        "release-list": [
                            {"title": "A Night at the Opera", "date": "1975", "release-group": {"type": "Album"}}
                        ],
            ...
            assert len(results) <= 5, "Should return max 5 results"
                ...
                "recording-list": [
                    {
                        ...
                        "release-list": [{"title": "Test Album", "date": "2020", "release-group": {"type": "Album"}}],
    ...
    def test_search_recording_with_artist_filter(self):
        ...
        with patch("musicbrainzngs.search_recordings") as mock_search:
            ...
            assert "artist" in call_args["query"].lower()
    ...
    def test_download_album_art_downloads_image(self):
        ...
        with patch("requests.get") as mock_get:
            ...
            assert self.test_output.exists(), "Image file should be created"
                ...
                "recording-list": [
                    {
                        ...
                        "artist-credit": [{"artist": {"name": "Artist"}}],
                        "release-list": [{"title": "Album", "date": "2020"}],
                        "tag-list": [{"name": "rock", "count": 10}, {"name": "progressive rock", "count": 5}],
                    }
                ]
            }
