...
class TestMetadataAutocompleter:
    ...
    def test_autocompleter_class_exists(self):
        ...
        if self.autocompleter_class is None:
            pytest.fail("MetadataAutocompleter class not found - implement src/core/metadata_autocompleter.py")
    ...
    def test_autocompleter_uses_musicbrainz_client(self):
        ...
        if self.autocompleter_class is None:
            pytest.fail("MetadataAutocompleter class not found")

        autocompleter = self.autocompleter_class()

        # Should have MusicBrainzClient instance
        assert hasattr(autocompleter, "mb_client")
        assert autocompleter.mb_client is not None
    ...
    def test_autocomplete_single_returns_matches(self):
        ...
        # Mock song data
        song_data = {"id": "song-123", "title": "Bohemian Rhapsody", "artist": "Queen"}
    ...
    def test_autocomplete_single_calculates_confidence(self):
        ...
        with patch.object(autocompleter.mb_client, "search_recording") as mock_search:
            mock_search.return_value = [
                {"title": "Test Song", "artist": "Test Artist", "album": "Album", "year": "2020", "genre": "pop"}
            ]
    ...
    def test_autocomplete_single_sorts_by_confidence(self):
        ...
        with patch.object(autocompleter.mb_client, "search_recording") as mock_search:
            mock_search.return_value = [
                {"title": "Song", "artist": "Artist", "album": "Album 1", "year": "2020", "genre": "pop"},
                {"title": "Song (Remix)", "artist": "Artist", "album": "Album 2", "year": "2021", "genre": "pop"},
                {"title": "Song", "artist": "Different Artist", "album": "Album 3", "year": "2019", "genre": "rock"},
            ]
    ...
    def test_autocomplete_batch_processes_multiple_songs(self):
        ...
        # Mock 5 songs
        songs = [{"id": f"song-{i}", "title": f"Song {i}", "artist": "Artist"} for i in range(5)]

        with patch.object(autocompleter.mb_client, "search_recording") as mock_search:
            mock_search.return_value = [
                {"title": "Song", "artist": "Artist", "album": "Album", "year": "2020", "genre": "pop"}
            ]
    ...
    def test_autocomplete_batch_auto_selects_high_confidence(self):
        ...
        with patch.object(autocompleter.mb_client, "search_recording", side_effect=mock_search):
            ...
            assert results["song-1"] is not None
            assert results["song-1"]["status"] == "auto_selected"

            # Low confidence match should require manual selection
            if results["song-2"] is not None:
                assert results["song-2"]["status"] == "manual_required"
    ...
    def test_autocomplete_batch_respects_limit(self):
        ...
        songs = [{"id": f"song-{i}", "title": f"Song {i}", "artist": "Artist"} for i in range(150)]
        ...
        try:
            ...
            assert "100" in str(e).lower() or "limit" in str(e).lower()
    ...
    def test_fuzzy_matching_improves_confidence(self):
        ...
        with patch.object(autocompleter.mb_client, "search_recording") as mock_search:
            mock_search.return_value = [
                {"title": "Song Title", "artist": "Artist", "album": "Album", "year": "2020", "genre": "pop"}
            ]
    ...
    def test_confidence_score_calculation_exact_match(self):
        ...
        mb_result = {"title": "Test Song", "artist": "Test Artist", "album": "Album", "year": "2020", "genre": "pop"}
