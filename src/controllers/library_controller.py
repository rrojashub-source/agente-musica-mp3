...
class LibraryController:
    ...
    def __init__(self, db_manager, status_bar=None):
    ...
    def set_widgets(self, tabs=None, library_tab=None, search_tab=None,
                    recommendations_widget=None, status_bar=None):
    ...
    def on_album_selected(self, album_data: dict):
    ...
    def update_recommendations(self, song_data: dict):
    ...
    def on_find_similar_requested(self, song_data: dict):
        """Handle find similar songs request — update Brain AI panel"""
        if self.recommendations_widget:
            self.recommendations_widget.set_current_song(song_data)
            if self.status_bar:
                self.status_bar.showMessage(
                    f"Finding similar to: {song_data.get('title', 'Unknown')}", 3000
                )

    def handle_focus_search(self):
    ...
    def handle_switch_tab(self, tab_name):
    ...
    def check_empty_library(self, parent_window):
