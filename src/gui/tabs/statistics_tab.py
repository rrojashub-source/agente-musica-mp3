...
class StatCard(QFrame):
    ...
    def __init__(self, title: str, value: str, subtitle: str = "", parent: Optional[QWidget] = None) -> None:
        ...
        title_label.setStyleSheet(Styles.LABEL_SMALL + " " + Styles.TEXT_DIMMED)
        ...
        value_label.setStyleSheet(Styles.TEXT_DARK)
        layout.addWidget(value_label)

        # Subtitle
        if subtitle:
            sub_label = QLabel(subtitle)
            sub_label.setStyleSheet(Styles.LABEL_TINY_FADED)
            layout.addWidget(sub_label)

        self.value_label: QLabel = value_label
        self.subtitle_label: Optional[QLabel] = sub_label if subtitle else None
...
class StatisticsTab(BaseTab):
    ...
    def __init__(self, db_manager: Any, parent: Optional[QWidget] = None) -> None:
        ...
        self.stats_service: StatisticsService = StatisticsService(db_manager)
    ...
    def _init_ui(self) -> None:
        ...
        for card in [
            ...
            self.total_artists_card,
            self.total_albums_card,
            self.total_duration_card,
        ...
        self.metadata_bars: Dict[str, QProgressBar] = {}
    ...
    def refresh_stats(self) -> None:
        ...
        try:
            ...
            self.total_songs_card.update_value(f"{overview.get('total_songs', 0):,}", "in library")
            self.total_artists_card.update_value(f"{overview.get('total_artists', 0):,}", "unique")
            self.total_albums_card.update_value(f"{overview.get('total_albums', 0):,}", "unique")
            ...
            self.total_plays_card.update_value(f"{overview.get('total_plays', 0):,}", "all time")

            # Update top artists
            top_artists = stats.get("top_artists", [])
            self.top_artists_widget.set_data(
                ["Artist", "Songs", "Plays"], [[a["artist"], a["song_count"], a["total_plays"]] for a in top_artists]
            )

            # Update top songs
            top_songs = stats.get("top_songs", [])
            self.top_songs_widget.set_data(
                ["Song", "Artist", "Plays"], [[s["title"], s["artist"], s["play_count"]] for s in top_songs]
            )

            # Update top genres
            top_genres = stats.get("top_genres", [])
            self.top_genres_widget.set_data(
                ["Genre", "Songs", "Plays"], [[g["genre"], g["song_count"], g["total_plays"]] for g in top_genres]
            )

            # Update decades
            decades = stats.get("decades", [])
            self.decades_widget.set_data(
                ["Decade", "Songs", "Plays"], [[d["decade"], d["song_count"], d["total_plays"]] for d in decades]
            )
            ...
            self.recent_added_widget.set_data(
                ["Song", "Artist", "Album"], [[s["title"], s["artist"] or "-", s["album"] or "-"] for s in recent_added]
            )

            # Update recently played
            recent_played = stats.get("recently_played", [])
            self.recent_played_widget.set_data(
                ["Song", "Artist", "Plays"], [[s["title"], s["artist"] or "-", s["play_count"]] for s in recent_played]
            )

            logger.info("Statistics refreshed successfully")

        except Exception as e:  # GUI error boundary - DB/query errors must not crash tab
