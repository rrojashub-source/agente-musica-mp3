...
class EqualizerPreset:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> 'EqualizerPreset':
...
class AudioEqualizer:
    ...
    def __init__(self, config_dir: str = None):
        ...
        self._bands: Dict[int, float] = {
            band.value: 0.0 for band in EqualizerBand
        }

        # Config directory for custom presets
        self._config_dir = Path(config_dir) if config_dir else Path('data')
        self._presets_file = self._config_dir / 'eq_presets.json'
    ...
    def set_band_gain(self, frequency: int, gain: float):
        """
    ...
    def _save_custom_presets(self):
        ...
        try:
            ...
            data = {
                key: preset.to_dict()
                for key, preset in self._custom_presets.items()
            }
